...
export function AuctionBiddersList({
  ...
  const { t } = useI18n();
  const listTitle = title ?? t("auctions.biddersList");
    ...
    >
      <div className="border-b border-slate-100 px-4 py-3">
        ...
        <p className="text-xs text-slate-500">
          {t("auctions.bidCount", "", { count: bids.length })}
        </p>
      </div>
      <div className="flex-1 overflow-y-auto p-3" style={{ maxHeight }}>
        {bids.length === 0 ? (
          <p className="py-12 text-center text-sm text-slate-500">{t("auctions.noBidsYet")}</p>
        ) : (
                ...
                <span className="min-w-0 truncate font-medium text-slate-800">
                  {i === 0 && (
                    <span className="ml-1.5 inline-block rounded bg-emerald-100 px-1.5 py-0.5 text-[10px] font-bold text-emerald-800">
                      {t("auctions.highestBid")}
                    </span>
                  )}
                  {b.bidderName || t("auctions.bidderFallback")}
                </span>
                <span className="shrink-0 font-bold text-emerald-700">
                  {formatPrice(b.bidAmount)} {t("common.currency")}
                </span>
              </li>
            ))}
          </ul>
